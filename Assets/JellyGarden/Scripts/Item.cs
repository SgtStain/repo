...
public class Item : MonoBehaviour
{
 ...
 //       print(remainColors.Count);
        ...
        if (exceptColor == randColor) randColor = (randColor++) % items.Length;
    ...
    IEnumerator AnimIdleStart()
    {
        ...
        while (true)
        {
            if (!trigger)
            {
                if (xScale > xScaleDest1) { xScale -= Time.deltaTime * speed; yScale += Time.deltaTime * speed; }
                else trigger = true;
            }
            else
            {
                if (xScale < xScaleDest2) { xScale += Time.deltaTime * speed; yScale -= Time.deltaTime * speed; }
                else trigger = false;
            }
    ...
    public void SwitchDirection(Vector3 delta)
    {
        ...
        {
            if (Mathf.Abs(deltaPos.x) > Mathf.Abs(deltaPos.y) && deltaPos.x > 0) switchDirection.x = 1;
            else if (Mathf.Abs(deltaPos.x) > Mathf.Abs(deltaPos.y) && deltaPos.x < 0) switchDirection.x = -1;
            else if (Mathf.Abs(deltaPos.x) < Mathf.Abs(deltaPos.y) && deltaPos.y > 0) switchDirection.y = 1;
            else if (Mathf.Abs(deltaPos.x) < Mathf.Abs(deltaPos.y) && deltaPos.y < 0) switchDirection.y = -1;
    ...
    IEnumerator Switching()
    {
        if (switchDirection != Vector3.zero && neighborSquare != null)
        {
            ...
            if (matchesInThisItem >= 4 ) thisItemHaveMatch = true;
            if (matchesInNeithborItem >= 4) thisItemHaveMatch = false;
            int matchesHereOneColor = matchesHere;
            matchesHere += matchesInNeithborItem;

            if ((this.currentType == ItemsTypes.BOMB || switchItem.currentType == ItemsTypes.BOMB) && (this.currentType != ItemsTypes.INGREDIENT && switchItem.currentType != ItemsTypes.INGREDIENT) ) matchesHere++;
            if (this.currentType > 0 && switchItem.currentType > 0) matchesHere++;
            if (this.currentType == ItemsTypes.INGREDIENT && switchItem.currentType == ItemsTypes.INGREDIENT) matchesHere = 0;
            ...
            if (matchesHere <= 0 && matchesInNeithborItem <= 0 && LevelManager.THIS.ActivatedBoost.type != BoostType.Hand || ((this.currentType == ItemsTypes.BOMB || switchItem.currentType == ItemsTypes.BOMB) && (this.currentType == ItemsTypes.INGREDIENT || switchItem.currentType == ItemsTypes.INGREDIENT)) || ((this.currentType == ItemsTypes.PACKAGE || switchItem.currentType == ItemsTypes.PACKAGE) && (this.currentType == ItemsTypes.INGREDIENT || switchItem.currentType == ItemsTypes.INGREDIENT)))
            {
            ...
            if (backMove) LevelManager.THIS.DragBlocked = false;
        }
        ResetDrag();
    }
    ...
    {
        if (item1.currentType == ItemsTypes.INGREDIENT || item2.currentType == ItemsTypes.INGREDIENT) return;
    ...
    {
//        print("set striped");
        if (Math.Abs(dir.x) > Math.Abs(dir.y)) nextType = ItemsTypes.HORIZONTAL_STRIPPED;
        else nextType = ItemsTypes.VERTICAL_STRIPPED;
    }
    ...
    IEnumerator FallingCor(Square _square, bool animate)
    {
        ...
        Vector3 startPos = transform.position;
        float speed = 10;
        if (LevelManager.THIS.gameStatus == GameState.PreWinAnimations) speed = 10;
    ...
    public bool GetNearEmptySquares()
    {
        ...
        {
            Square checkingSquare = LevelManager.Instance.GetSquare(square.col + 1, square.row , true);
   ...
   //     square.DestroyBlock();
    ...
    public void DestroyColor(int p)
    {
        ...
        {
            if (item.GetComponent<Item>().color == p) item.GetComponent<Item>().DestroyItem(true, "", true);
        }
    }
