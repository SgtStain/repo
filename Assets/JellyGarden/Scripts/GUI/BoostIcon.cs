...
public class BoostIcon : MonoBehaviour
{
    ...
    {
        if (LevelManager.THIS.ActivatedBoost == this) { UnCheckBoost(); return; }
        if (IsLocked() || check || (LevelManager.THIS.gameStatus != GameState.Playing && LevelManager.THIS.gameStatus != GameState.Map)) return;
        if (BoostCount() > 0)
        {
    ...
    void Update()
    {
        if (boostCount != null)
        {
            ...
            if (!check)
            {
                if (BoostCount() > 0) ShowPlus(false);
                else ShowPlus(true);
            }
        }
    }
