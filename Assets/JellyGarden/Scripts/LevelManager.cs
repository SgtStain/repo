...
public class LevelManager : MonoBehaviour
{
    ...
    private BoostIcon activatedBoost;
    ...
    public int TargetBlocks
    {
        ...
        set
        {
            if (targetBlocks < 0) targetBlocks = 0;
            targetBlocks = value;
        }
    }
    ...
    public GameState gameStatus
    {
        ...
        set 
        {
            ...
            else if (value == GameState.Win)
            {
                ...
                {
                    if (passLevelCounter % InitScript.Instance.ShowChartboostAdsEveryLevel == 0 && InitScript.Instance.enableAds) InitScript.Instance.ShowAds(true);
                }
                if (InitScript.Instance.ShowAdmobAdsEveryLevel > 0)
                {
                    if (passLevelCounter % InitScript.Instance.ShowAdmobAdsEveryLevel == 0 && InitScript.Instance.enableAds) InitScript.Instance.ShowAds(false);
                }
    ...
    void LockBoosts()
    {
        ...
        {
            if (item != ActivatedBoost) item.LockBoost();
        }
    }
    ...
    public void EnableMap(bool enable)
    {
        ...
        if (enable)
        {
            ...
            if (aspect == 1.6f) GetComponent<Camera>().orthographicSize = 6.25f;                    //16:10
            else if (aspect == 1.777777777777778f) GetComponent<Camera>().orthographicSize = 7f;    //16:9
            else if (aspect == 1.5f) GetComponent<Camera>().orthographicSize = 5.9f;                  //3:2
            else if (aspect == 1.333333333333333f) GetComponent<Camera>().orthographicSize = 5.25f;                  //4:3
            else if (aspect == 1.666666666666667f) GetComponent<Camera>().orthographicSize = 6.6f;                  //5:3
            else if (aspect == 1.25f) GetComponent<Camera>().orthographicSize = 4.9f;                  //5:4
    ...
    void InitTargets()
    {
        ...
        if (ingrCountTarget[0] == 0 && ingrCountTarget[1] == 0) ingrObject.SetActive(false);
   ...
   //     SoundBase.Instance.audio.PlayOneShot(SoundBase.Instance.getStarIngr);
    ...
    IEnumerator PreWinAnimationsCor()
    {
        ...
        List<Item> items = GetRandomItems(8);
        foreach (Item item in items)
        {
 ...
 //           GenerateNewItems();
    ...
    void SetOutline(int col, int row, float zRot)
    {
        ...
        if (square.type != SquareTypes.NONE)
        {
            if (row == 0 || col == 0 || col == maxCols - 1 || row == maxRows - 1)
            {
                ...
                if (zRot == 0) outline.transform.localPosition = Vector3.zero + Vector3.left * 0.425f;
                if (zRot == 90) outline.transform.localPosition = Vector3.zero + Vector3.down * 0.425f;
                if (zRot == 180) outline.transform.localPosition = Vector3.zero + Vector3.right * 0.425f;
                if (zRot == 270) outline.transform.localPosition = Vector3.zero + Vector3.up * 0.425f;
    ...
    IEnumerator RegenMatches(bool onlyFalling = false)
    {
     ...
     //   yield return new WaitForSeconds(1f);
        ...
        if (gameStatus == GameState.RegenLevel) gameStatus = GameState.Playing;
 ...
 //       List<Item> countedSquares = new List<Item>();
    ...
    IEnumerator FallingDown()
    {
        ...
        while (true)
        {
            ...
            if (combinedItems.Count > 0) combo++;
            ...
            if (destroyAnyway.Count > 0) nearEmptySquareDetected = true;
            if (GetMatches().Count <= 0 && !nearEmptySquareDetected) break;
        }
        ...
        if (!thrivingBlockDestroyed)
        {
            ...
            for (int col = 0; col < maxCols; col++)
            {
                if (thrivingBlockSelected) break;
                for (int row = maxRows - 1; row >= 0; row--)
                {
                    if (thrivingBlockSelected) break;
                    if (GetSquare(col, row) != null)
    ...
    {
        if (gameStatus == GameState.PreWinAnimations) return true;
        GameObject[] items = GameObject.FindGameObjectsWithTag("Item");
        foreach (GameObject item in items)
        {
            Item itemComponent = item.GetComponent<Item>();
    ...
    {
        if (exceptList == null) exceptList = new List<Item>();
        int c = square.col;
        int r = square.row;
        List<Item> itemsList = new List<Item>();
        Item item = null;
    ...
    public void StrippedShow(GameObject obj, bool horrizontal)
    {
        ...
        if (!horrizontal) effect.transform.Rotate(Vector3.back, 90);
        Destroy(effect, 1);
    }
    ...
    void CheckStars()
    {
        ...
        if (Score >= star1) 
        {
            if (!star1Anim.activeSelf) SoundBase.Instance.GetComponent<AudioSource>().PlayOneShot(SoundBase.Instance.getStarIngr);
            star1Anim.SetActive(true); 
        }
        if (Score >= star2)
        {
            if (!star2Anim.activeSelf) SoundBase.Instance.GetComponent<AudioSource>().PlayOneShot(SoundBase.Instance.getStarIngr);
            star2Anim.SetActive(true);
        }
        if (Score >= star3)
        {
            if (!star3Anim.activeSelf) SoundBase.Instance.GetComponent<AudioSource>().PlayOneShot(SoundBase.Instance.getStarIngr);
